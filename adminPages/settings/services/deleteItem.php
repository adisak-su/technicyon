...
try {
	if (isset($_POST["itemId"]) && !empty($_POST["itemId"])) {
		...
		// $sql = "SELECT colorname FROM colorname WHERE colorNo=$itemId";
		...
		$sql = "DELETE FROM colorname WHERE colorNo=:itemId";
		$stmt = $conn->prepare($sql);
		$stmt->execute($params);
...
function updateUsercar($itemColorNameOld, $conn, $DB)
{
	try {
		...
		$sql = "SELECT usercarId FROM usercar WHERE colorname = :itemColorNameOld";
		$stmt = $conn->prepare($sql);
		$stmt->execute($params);
