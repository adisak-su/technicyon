...
<head>
    ...
    <title>ระบบจัดการประเภทสินค้า | <?php echo $shopName; ?></title>
...
<body class="sidebar-collapse">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-12 form-group position-relative mb-3">
                                        <input
                                            ...
                                            id="itemId"
                                            placeholder="รหัส..." />
                                        <label for="itemName" class="form-label">ประเภทสินค้า</label>
                                        <div class="input-icon-wrapper">
    ...
    <script type="text/javascript">
        ...
        async function _loadAndSetData(storeName) {
            ...
            typeNames = dataStore;
            // typeNames = dataStore.map((item,index) => { return {"index":index , "typename":item.typename,"updatedAt":item.updatedAt}; });
        }
        ...
        async function syncDataRealtimeServer() {
            let dataExpires = [];
            try {
                dataExpires = await updateSyncData(["typename"]);
            } catch (error) {
                ...
                if (dataExpires.length) {
                    ...
                    dataExpires.forEach((item) => {
                        switch (item.tableName) {
                            case "typename":
                                typeNames = sortColumnData(item.data);
                                createFilterDataAndRender(currentPage);
                                break;
                        }
                    });
