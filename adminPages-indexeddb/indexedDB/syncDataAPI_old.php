...
<body>
    <div class="container text-center my-5">
        <div id="statusBar" class="bg-secondary text-white py-2 text-center position-relative">
            <span id="statusIcon" class="mb-5">🔄</span> <span id="statusText">กำลังโหลดข้อมูล...</span>
    ...
    <script>
        ...
        async function syncStoreFromAPI(storeName) {
            ...
            let html = `<div id="syncName_${storeName}">
                        <div class="text-white mt-3">ข้อมูล : ${storeName}</div>
...
/*
        async function syncStorsFromAPI(storeNames) {
            return new Promise((resolve, reject) => {
                    const start = Date.now();
                    let html = `<div id="syncName_${storeName}">
                        <div class="text-white mt-3">ข้อมูล : ${storeName}</div>
...
*/
        async function syncFromAPI() {
            ...
            try {
                ...
                // let result = await syncStoresFromAPI(storeNames);

                await storeNames.forEach((store) => {
                    storeName = store.storeName;
                    let result = syncStoreFromAPI(storeName);
                });

                const tx = db.transaction(metaStore, "readwrite");
                const meta = tx.objectStore(metaStore);
                const now = new Date().addHours(7).toISOString().replace("T", " ").substr(0, 19);
                // alert(now);
