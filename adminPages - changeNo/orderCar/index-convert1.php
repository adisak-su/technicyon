...
$columnName["usercar"] = "(carId,type,color,name,address,telephone,mile,birthday)";
...
$columnNameMapper = [];
$columnNameMapper["product"] = "(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$columnNameMapper["usercar"] = "(?,?,?,?,?,?,?,?)";
$columnNameMapper["groupname"] = "(?)";
$columnNameMapper["typename"] = "(?)";
$columnNameMapper["customer"] = "(?,?,?,?,?,?)";
$columnNameMapper["supplier"] = "(?,?,?,?)";
$columnNameMapper["usercar_head"] = "(?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
$connection = odbc_connect("DBConvert", "", "");
if (!$connection) {
    echo (die(odbc_error()));
} else {
...
{
    global $connectionMySQL, $connection, $columnName, $columnNameMapper;
    ...
    try {
        ...
        $columnMappers = $columnNameMapper[$tableName];
        print_r($columnMappers);

        // เตรียมคำสั่ง SQL
        $sql = "INSERT INTO $tableName $columns VALUES $columnMappers";
        $stmt = $connectionMySQL->prepare($sql);

        // เริ่ม transaction สำหรับประสิทธิภาพ
        $connectionMySQL->beginTransaction();

        // เพิ่มข้อมูลทีละรายการ
        while ($arr = odbc_fetch_array($data)) {
            $countRow++;
            // for($index=0;$index < count($arr);$index++) {
            //     $arr[$index] = iconv("Windows-874", "utf-8", trim($arr[$index]));
            // }
            // foreach ($arr as $col) {
            //     $col = iconv("Windows-874", "utf-8", trim($col));
            // }
            // $stmt->execute($arr);
            $item = [];
            foreach ($arr as $col) {
