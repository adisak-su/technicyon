...
function openIndexedDB() {
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = e => {
      ...
      if (!db.objectStoreNames.contains(storeName)) db.createObjectStore(storeName, { keyPath: "productId" });
...
async function _syncFromAPI() {
  ...
  try {
    ...
    const res = await fetch("services/products.php");
...
async function saveProductToAPI(product) {
  if (navigator.onLine) {
    try {
      // alert(product);
      console.log(product);
      await fetch("services/saveProduct.php", {
        method: "PUT",
        // method: "POST",
...
async function saveProduct(e) {
  ...
  const product = { productId, productName, price, updatedAt };
  const db = await openIndexedDB();
...
async function deleteProduct(id) {
  ...
  if (navigator.onLine) {
    try {
      // await fetch(`services/product.php?id=${id}`, { method: "DELETE" });
      await fetch("services/deleteProduct.php", {
        // method: "PUT",
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
...
function renderTable(data) {
  ...
  data.forEach(p => {
    body.innerHTML += `<tr>
      <td>${p.productId}</td><td>${p.productName}</td><td>${p.price}</td>
      <td>${new Date(p.updatedAt).toLocaleString()}</td>
      <td>
        <button class='btn btn-sm btn-warning' onclick='openEditModal(${JSON.stringify(p)})'>แก้ไข</button>
        <button class='btn btn-sm btn-danger' onclick='confirmDelete(${p.productId})'>ลบ</button>
      </td></tr>`;
  });
}
